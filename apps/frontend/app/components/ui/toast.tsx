...
>(({ className, variant = 'default', ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(
        'group pointer-events-auto relative flex w-full max-w-md items-center gap-4 overflow-hidden rounded-xl border p-4 shadow-lg transition-all',
        ...
        variant === 'default' &&
          'bg-gray-900/95 backdrop-blur-md border-gray-800/60',
        variant === 'destructive' &&
      ...
      {toasts.map(function ({ id, title, description, action, ...props }) {
        return (
          <Toast
            ...
            {...props}
            className={cn(
              'fixed top-4 left-1/2 transform -translate-x-1/2 w-auto min-w-[320px] max-w-[420px]',
              props.className
            )}
          >
