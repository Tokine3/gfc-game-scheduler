...
export const OpenCalendarDialog: FC<Props> = ({
  ...
  useEffect(() => {
    if (isOpen && calendar) {
      router.prefetch(`/calendar/${calendar.id}`);
    }
  }, [isOpen, calendar, router]);

  const handleConfirm = async (calendarId: string) => {
  ...
  return (
    <>
      <Dialog open={isOpen && !isTransitioning} onOpenChange={onClose}>
